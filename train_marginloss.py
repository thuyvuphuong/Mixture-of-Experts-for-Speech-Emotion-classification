...
from tensorflow.keras.losses import categorical_crossentropy, categorical_hinge
import argparse
...
parser.add_argument('--alpha', type=float, default=0.5, help='Loss of weights')
...
x_train, x_test, y_train, y_test = train_test_split(voices_mel_expanded, labels_onehot, test_size=0.1, random_state=42, shuffle=True)
print(x_train.shape)
print(x_test.shape)
print(y_train.shape)
print(y_test.shape)
...
moes_layer = MoE2(input_shape=inputs.shape, num_experts=num_experts, 
                  spa_filters=64, top_k=top_k, dropout_rate=0)
...
# %%
checkpoint_filepath = f'trained_models/2losses_model_sp{num_experts}_top{top_k}.keras'
acc_save_path = f'../trained_output/2losses_acc_and_loss_sp{num_experts}_top{top_k}.jpg'
cfs_matrix_save_path = f'../trained_output/2losses_confusion_matrix_sp{num_experts}_top{top_k}.jpg'
...
class CustomLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        hinge_loss = categorical_hinge(y_true, y_pred)
        return self.alpha * cce_loss + (1-self.alpha) * hinge_loss

callbacks = get_callbacks(patience = patience)
optimizer = tf.keras.optimizers.AdamW(learning_rate=lr)
metrics = ['accuracy']
moes_model.compile(optimizer, 
                   loss= CustomLoss(alpha=alpha), 
                   metrics=metrics)
